...
//                   <p className='mx-1'>{selectedUser?.lastName}</p>
//                 </div>
//                 <i>{selectedUser?.bio || "No Boi"}</i>
//               </div>
//             </div>
//           </div>
...
export default function MessageContainer() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_KEY}/api/message/send/${selectedUser?._id}`,
        { message },
  ...
  return (
    <div
      className={
        visible
          ? `bg-slate-300 h-full flex flex-col py-4 px-2 sm:px-4 w-full sm:w-2/3 md:w-3/4 lg:w-4/5`
          : `hidden sm:flex flex-col bg-slate-300 h-full py-4 px-2 sm:px-4 w-full sm:w-2/3 md:w-3/4 lg:w-4/5`
      }
    >
      {selectedUser?._id ? (
        <>
          ...
          <div className='flex flex-row items-center mb-4'>
            <button
              ...
              className={
                visible
                  ? `sm:hidden flex justify-center items-center text-2xl bg-gray-400 rounded-md p-2 mr-2`
                  : `hidden`
              }
            >
              <MdArrowBackIos />
            ...
            <div className='flex items-center bg-gray-400 rounded-md p-2 w-full'>
              <img
                src={selectedUser?.image || profilepic}
                alt="profile pic"
                className='h-8 w-8 border-2 rounded-full mr-3'
              />
              <div>
                <div className='flex'>
                  <p>{selectedUser?.firstName || selectedUser?.email}</p>
                ...
                <i>{selectedUser?.bio || "No Bio"}</i>
              </div>
            </div>
          </div>

          {/* Messages Area */}
          <div className='flex-1 overflow-y-auto px-1 pr-3 no-scrollbar'>
            {selectedUser ? <Messages /> : <div>Start Chatting</div>}
          </div>
          ...
          <form
            ...
            className='flex items-center border-t-2 border-gray-500 pt-3 mt-3'
          >
            <input
              type="text"
              value={message}
            ...
            <button
              type='submit'
              className='bg-gray-400 py-2 px-3 rounded-md ml-2 text-white hover:bg-gray-500'
            >
        ...
        <div className='flex flex-col justify-center items-center h-full font-semibold space-y-3 text-center'>
          <div>Welcome <span className='text-2xl text-red-600'>{authUser?.email}</span></div>
          <div className='text-red-400'>Start chatting with your friends..</div>
        </div>
      )}
